...
template <typename O>
class CMaxInvCreditByCallsMultiOperator: public AMultiOperator<O>
{
public:
    virtual CAtomicOperator<O>* SelectOperator()
    {
        ...
        else
        {
            ...
            for (CAtomicOperator<O>& atomicOperator : this->m_AtomicOperators)
            {
                ...
                float operatorScore = (1.f / ((float)operatorData.m_Credits + 1.f)) / ((float)operatorData.m_Calls + 1.f);
