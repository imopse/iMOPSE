...
def run_executable(config_string, silent):
    command = f"imopse.exe {config_string}"
    ...
    if result.returncode != 0 or not silent:
        # Print the output. Removed ANSI color codes as they may not work in Windows CMD
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run build\\imopse.exe with configurations')
